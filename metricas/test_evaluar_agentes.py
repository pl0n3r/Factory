...
from metricas.evaluar_agentes import (
    ValidationError,
    aggregate,
    load_records,
    render_markdown,
...
class EvaluationTests(unittest.TestCase):
    def test_ranking_is_separate_by_task_type_and_deterministic(self):
        records = [
            ...
            validate_record(
                record("r#5", task_type="frontend", model="luna"),
                "5",
            ),
            validate_record(
                record("r#6", task_type="frontend", model="luna"),
                "6",
            ),
            validate_record(
                record(
        ...
        backend = report["task_types"]["backend"]
        frontend = report["task_types"]["frontend"]
        self.assertEqual(
            backend["configurations"][0]["configuration"]["model"],
            "sol",
        )
        self.assertEqual(backend["configurations"][0]["rank"], 1)
        self.assertIsNotNone(backend["recommendation"])
        self.assertEqual(
            {row["rank"] for row in frontend["configurations"]},
            {1, 2},
        )

    def test_insufficient_sample_never_fabricates_recommendation(self):
        report = aggregate(
            [validate_record(record("r#1"), "1")],
            min_samples=2,
        )
        ...
        self.assertEqual(
            data["configurations"][0]["sample_status"],
            "insufficient_data",
        )
    ...
    def test_exact_quality_tie_has_no_recommendation(self):
        ...
        for index in range(1, 4):
            records.append(
                validate_record(
                    record(f"r#{index}", model="sol"),
                    f"sol-{index}",
                )
            )
            records.append(
                validate_record(
                    record(f"r#{index + 10}", model="luna"),
                    f"luna-{index}",
                )
            )
        ...
        self.assertEqual(
            ...
            "tie-or-incomplete-cost",
        )
        self.assertEqual(
            {row["rank"] for row in data["configurations"]},
            {1},
        )
    ...
    def test_partial_token_telemetry_never_becomes_zero_cost(self):
        records = [
            validate_record(
                record("r#1", model="sol", tokens_output=None),
                "1",
            ),
            validate_record(record("r#2", model="sol"), "2"),
        ...
        report = aggregate(records, min_samples=2)
        rows = {
            row["configuration"]["model"]: row
            for row in report["task_types"]["backend"]["configurations"]
        }
        self.assertIsNone(rows["sol"]["tokens_avg"])
        self.assertEqual(rows["sol"]["tokens_complete_samples"], 1)
        backend = report["task_types"]["backend"]
        self.assertIsNone(backend["recommendation"])
        self.assertEqual(
            backend["recommendation_status"],
            "tie-or-incomplete-cost",
        )
    ...
    def test_same_input_produces_same_report(self):
        ...
        first = aggregate(records, min_samples=1)
        second = aggregate(list(records), min_samples=1)
        self.assertEqual(first, second)

    def test_rejects_unknown_fields_and_review_rounds_over_limit(self):
        unknown = record("r#1", unexpected_field="no")
        too_many_rounds = record("r#2", review_rounds=4)
        with self.assertRaisesRegex(
            ValidationError,
            "campos no permitidos",
        ):
            validate_record(unknown, "fixture")
        with self.assertRaisesRegex(
            ValidationError,
            "no puede superar 3",
        ):
            validate_record(too_many_rounds, "fixture")

    def test_rejects_free_text_identity_fields(self):
        free_text = record("repo#9", prompt_id="prompt con texto")
        email_like = record("correo@example.com")
        with self.assertRaisesRegex(ValidationError, "identificador"):
            validate_record(free_text, "fixture")
        with self.assertRaisesRegex(ValidationError, "repo#N"):
            validate_record(email_like, "fixture")
    ...
    def test_month_filter_uses_utc_boundaries(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rows = [
                ...
                record(
                    "r#12",
                    completed_at="2026-10-01T00:30:00+01:00",
                ),
            ...
            self.assertEqual(
                [row["task_id"] for row in september],
                ["r#10", "r#11", "r#12"],
            )
    ...
    def test_empty_directory_is_allowed_only_when_explicit(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                load_records([directory], allow_empty=True),
                [],
            )
