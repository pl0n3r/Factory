...
El successor de una renovación v2/v3 se deriva de forma determinista del Issue, UUID anterior, fingerprint de aceptación, fingerprint de task (o ausencia explícita) y HEAD exacto. El retry revalida esos mismos inputs antes de aceptar un successor histórico y reconcilia idempotentemente solo los estados old/old, old/new, new/old y new/new. Se publica **un solo `Validar` failure** por transición: si el PR ya porta el successor determinista, el retry no crea otro. Una tercera sesión o cualquier drift de contrato/claims/HEAD falla cerrado y nunca se sobrescribe.\n\nAnte fallo o carrera, se conserva una combinación old/new reintentable o se falla cerrado con `Validar` invalidado; no se destruyen rama ni PR. `/liberar` + `/tomar` sigue disponible para abandonar el trabajo, pero cierra PR y rama. Nunca usar `/renovar-contrato` para ocultar una revisión fallida ni para eludir una decisión humana.
