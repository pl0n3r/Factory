...
def _timestamp(value: Any, field: str) -> datetime:
    if not isinstance(value, str):
        raise FeedbackValidationError(f"{field} debe ser ISO-8601 con zona horaria.")
    try:
...
def load_catalog(path: Path = CATALOG_PATH) -> dict[str, dict[str, str]]:
    try:
        ...
        raise FeedbackValidationError("Catálogo de métricas ilegible o inválido.") from exc
    ...
    for metric, spec in metrics.items():
        ...
        if not isinstance(spec, dict) or set(spec) != {"family", "unit", "direction"}:
            raise FeedbackValidationError(
                f"Métrica {metric} debe declarar family, unit y direction."
            )
        family, unit, direction = spec["family"], spec["unit"], spec["direction"]
        if family not in FAMILIES or unit not in UNITS or direction not in DIRECTIONS:
            raise FeedbackValidationError(f"Contrato inválido para métrica {metric}.")
        catalog[metric] = {
            "family": family,
            "unit": unit,
            "direction": direction,
...
def validate_record(
    ...
    if not isinstance(metric, str) or metric not in catalog:
        raise FeedbackValidationError(f"{where}: metric no pertenece al catálogo.")
    ...
    if phase not in PHASES:
        raise FeedbackValidationError(f"{where}: phase debe ser baseline o post-deploy.")

    start = _timestamp(raw["period_start"], f"{where}: period_start")
    end = _timestamp(raw["period_end"], f"{where}: period_end")
    if end <= start:
        raise FeedbackValidationError(f"{where}: period_end debe ser posterior a period_start.")
    ...
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise FeedbackValidationError(f"{where}: value debe ser numérico finito.")
    ...
    if (
        ...
        or not 1 <= sample_size <= MAX_SAMPLE_SIZE
    ):
        raise FeedbackValidationError(
            f"{where}: sample_size debe estar entre 1 y {MAX_SAMPLE_SIZE}."
        )

    unit = catalog[metric]["unit"]
    if unit == "ratio" and value > 1:
        raise FeedbackValidationError(f"{where}: ratio debe estar entre 0 y 1.")
    if unit == "count" and (isinstance(value, bool) or not isinstance(value, int)):
...
def load_records(
    ...
    if not directory.exists():
        if allow_empty:
            return []
        raise FeedbackValidationError("Directorio de telemetría inexistente.")
    if not directory.is_dir() or directory.is_symlink():
        raise FeedbackValidationError("La telemetría debe provenir del directorio canónico.")
    ...
    for path in files:
        ...
        try:
            ...
            raise FeedbackValidationError(f"{path.name}: no se pudo abrir.") from exc
        ...
        try:
            for line_no, line in enumerate(handle, 1):
                ...
                record = validate_record(raw, catalog, where)
                identity = (
                    record["project"], record["epic"], record["metric"],
                    record["surface"], record["phase"],
                    record["period_start"], record["period_end"],
                )
                if identity in seen:
                    raise FeedbackValidationError(
                        f"{where}: agregado duplicado; no se contabiliza dos veces."
                    )
                seen.add(identity)
    ...
    if not records and not allow_empty:
        raise FeedbackValidationError("No se encontraron registros de telemetría.")
    return records
...
def parse_epic_marker(
    ...
    if not isinstance(body, str):
        raise FeedbackValidationError("El cuerpo del épico debe ser texto.")
    intent = MARKER_NAME in body
    matches = MARKER_RE.findall(body)
    if not matches:
        if intent:
            raise FeedbackValidationError("Marker factory-product-metric malformado.")
        return None
    if len(matches) != 1:
        raise FeedbackValidationError("Debe existir un único marker factory-product-metric.")
    try:
        raw = json.loads(matches[0])
    except json.JSONDecodeError as exc:
        raise FeedbackValidationError("Marker factory-product-metric contiene JSON inválido.") from exc
    ...
    metric = raw["metric"]
    surface = raw["surface"]
    target = raw["target_improvement_pct"]
    if not isinstance(metric, str) or metric not in catalog:
        raise FeedbackValidationError("Marker usa una métrica fuera del catálogo.")
    if not isinstance(surface, str) or not SLUG_RE.fullmatch(surface):
        raise FeedbackValidationError("Marker usa surface inválida.")
    if isinstance(target, bool) or not isinstance(target, (int, float)):
        raise FeedbackValidationError("target_improvement_pct debe ser numérico finito.")
    if isinstance(target, float) and not math.isfinite(target):
        raise FeedbackValidationError("target_improvement_pct debe ser numérico finito.")
    if target <= 0 or target > 1000:
        raise FeedbackValidationError("target_improvement_pct debe estar en (0, 1000].")
    return {
        "metric": metric,
        "surface": surface,
...
def load_epic_markers(
    ...
    if size > MAX_MARKER_FILE_BYTES:
        raise FeedbackValidationError("Mapa de markers demasiado grande.")
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise FeedbackValidationError("Mapa de markers ilegible o inválido.") from exc
    ...
    for epic in sorted(required):
        ...
        if not isinstance(body, str) or len(body) > 200_000:
            raise FeedbackValidationError(f"{epic}: body del épico inválido.")
...
def _validate_phase_windows(
    ...
    for phase_name, rows in (("baseline", baseline_rows), ("post-deploy", post_rows)):
        ...
        for row in ordered:
            ...
            if previous_end is not None and start < previous_end:
                raise FeedbackValidationError(
                    f"Ventanas {phase_name} solapadas; la muestra se contaría dos veces."
                )
            previous_end = end
        if family == "cwv" and len(rows) > 1:
            raise FeedbackValidationError(
                "CWV p75 requiere un único agregado por fase; no se promedian percentiles."
            )

    if baseline_rows and post_rows:
        baseline_end = max(_period(row["period_end"]) for row in baseline_rows)
        post_start = min(_period(row["period_start"]) for row in post_rows)
...
def _weighted_average(
    ...
    if total_samples <= 0:
        raise FeedbackValidationError("Grupo sin muestra; no se puede promediar.")
    if unit == "count":
        total_days = sum(
            (_period(row["period_end"]) - _period(row["period_start"])).total_seconds()
            / 86_400
            for row in rows
        )
        if total_days <= 0:
            raise FeedbackValidationError(
                "Métrica count requiere duración positiva para calcular tasa diaria."
            )
        value = sum(row["value"] for row in rows) / total_days
    else:
        value = (
            sum(row["value"] * row["sample_size"] for row in rows)
            / total_samples
        )
...
def aggregate(
    ...
    if isinstance(min_sample_size, bool) or not isinstance(min_sample_size, int) or min_sample_size < 1:
        raise FeedbackValidationError("min_sample_size debe ser entero >= 1.")
    ...
    for (project, epic, metric, surface), phases in sorted(groups.items()):
        ...
        row: dict[str, Any] = {
            ...
            "baseline_samples": sum(item["sample_size"] for item in baseline_rows),
            "post_deploy_samples": sum(item["sample_size"] for item in post_rows),
        }

        if baseline_rows:
            row["baseline"], row["baseline_samples"] = _weighted_average(
                baseline_rows, spec["unit"]
            )
        if post_rows:
        ...
        if enough and row["baseline"] == 0:
            ...
            improvement_pct = (
                delta_pct if spec["direction"] == "increase" else -delta_pct
            )
            ...
            if row["status"] == "regressed":
                ...
                proposals.append({
                    ...
                    "suggested_title": f"[AUTO] revisar {metric} en {surface}",
                })
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "",
        "| Épico | Familia | Métrica | Surface | Objetivo | Estado | Baseline | Post | Δ |",
        "| --- | --- | --- | --- | ---: | --- | ---: | ---: | ---: |",
    ]
    for row in report["results"]:
        delta = "—" if row["delta_pct"] is None else f"{row['delta_pct']:.2f}%"
        baseline = "—" if row["baseline"] is None else row["baseline"]
        post = "—" if row["post_deploy"] is None else row["post_deploy"]
        lines.append(
    ...
    if not report["proposals"]:
        lines += ["Sin regresiones medibles que justifiquen una propuesta automática.", ""]
    else:
        for index, item in enumerate(report["proposals"], 1):
            lines += [
                f"{index}. **{item['suggested_title']}**",
                f"   - Épico: {item['epic']}",
                f"   - Objetivo declarado: {item['target_improvement_pct']:.2f}%",
                f"   - Regresión: {item['regression_pct']:.2f}% · muestra post: {item['post_deploy_samples']}",
                f"   - Impact score auditable: {item['impact_score']:.2f}",
            ]
        lines.append("")
    return "\n".join(lines)
...
def main() -> int:
    ...
    try:
        ...
        if args.list_epics:
            ...
            return 0
        markers = load_epic_markers(EPIC_MARKERS_PATH, catalog, epics)
        report = aggregate(
            records,
            catalog,
    ...
    try:
        ...
        print("ERROR: no se pudo escribir la evidencia semanal.", file=sys.stderr)
        return 2
    return 0
